...
class State(rx.State):
    ...
    def add_libro(self):
        if self.new_libro and self.new_costo and self.new_imagen:
            self.libros.append(Libro(titulo=self.new_libro, costo = self.new_costo, imagen=self.new_imagen))
            self.new_libro = ''
            self.new_costo = 0.0
            self.new_imagen =  ''
...
def item_libros(libro: Libro, index: int):
    return rx.hstack(
        rx.vstack(
            rx.text('titulo:'),
            rx.heading(libro.titulo, color_scheme='blue', font_family='Times New Roman'),
        ),
        rx.vstack(
            rx.heading('S/. ', libro.costo, color_scheme='green', font_family='Times New Roman'),
            rx.text('imagen:'),  # Rótulo para la imagen
            rx.image(libro.imagen, alt=libro.titulo),  # Componente de imagen
        ),
        rx.button( 
            "Borrar",
...
def item_carrito(libro:Libro, index: int):
    return rx.hstack(
        ...
        rx.vstack(
            ...
            rx.text('imagen:'),
            rx.image(libro.imagen,alt=libro.titulo),
        ),
            ...
            rx.vstack(
                ...
                rx.input(
                 ...
                 rx.input(
                    ...
                    type = 'text',
                    value=State.new_imagen,
                    on_change=State.set_new_imagen,
                ),
